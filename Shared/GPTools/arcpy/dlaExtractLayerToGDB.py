...
def exportDataset(xmlDoc, source, workspace, targetName, rowLimit, datasetType):
    ...
    if str(srcCount) == '0':
        ...
        if str(count) == '0':
            result = False
            dla.addError(
                "Failed to load to " + targetName + ", it is likely that your data falls outside of the target Spatial Reference Extent or there is another basic issue")
            dla.addError(
...
def getFieldMap(view, ds):
    ...
    inFields = [field.name for field in arcpy.ListFields(view) if
                field.name.upper() not in dla._ignoreFields]  # not field.required removed after .Enabled issue
    removenames = []
    for i in range(fieldMaps.fieldCount):
        ...
        try:
            ...
            try:
                ...
                f = inFields.index(fName.replace('_', '.',
                                                 1))  # just replace the first char - more complex case like xfmr.phase_designation
            if f > -1:
                inField = inFields[f]
...
def createDataset(dsType, workspace, targetName, geomType, xmlDoc, source, targetRef):
    if source.lower().endswith('.lyrx') and dla.hasJoin(source):
        if dsType == 'Table':
            ...
            arcpy.CreateFeatureclass_management(workspace, targetName, geometry_type=geomType,
        ...
        for sfield in sourceFields:
            ...
            if fname.count('.') > 0:
                ...
                dla.addDlaField(os.path.join(workspace, targetName), fname, sfield, [], ftype,
                                flength)  # attrs is empty list
    else:
        if dsType == 'Table':
            try:
...
def removeDefaultValues(dataset):
    ...
    for sfield in sourceFields:
        ...
        if sfield.defaultValue != None:
            try:
                arcpy.AssignDefaultToField_management(in_table=dataset, field_name=fname, default_value=None,
                                                      clear_value=True)  # clear the Defaults
            except:
                dla.addMessage(
                    "Unable to set DefaultValue for " + fname)  # skip GlobalIDs/other fields that cannot be updated. Should not have a default set in these cases
